...
```
wget https://github.com/SirenKeter/Onelist/raw/main/%40Theme/HaorWu/index.html

wget https://github.com/SirenKeter/Onelist/raw/main/amd64/linux/OneList
```
...
<a href="https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=78d4dc35-7e46-42c6-9023-2d39314433a5&response_type=code&redirect_uri=http://localhost/onedrive-login&response_mode=query&scope=offline_access%20User.Read%20Files.ReadWrite.All">国际版, 个人版(家庭版)</a>           

<a href="https://login.chinacloudapi.cn/common/oauth2/v2.0/authorize?client_id=dfe36e60-6133-48cf-869f-4d15b8354769&response_type=code&redirect_uri=http://localhost/onedrive-login&response_mode=query&scope=offline_access%20User.Read%20Files.ReadWrite.All">中国版(世纪互联)</a>
...
```
# 以下均为在OneList文件目录下运行的命令
# 确保OneList、config.json 和 index.html均在此目录, 直接运行.
./OneList
# 监听公网 5288 端口
nohup ./OneList -bind 0.0.0.0 -port 5288 >/dev/null 2>&1 &
```
...
# 后台运行
nohup /path/to/OneList -bind 0.0.0.0 -port 5288 >/dev/null 2>&1 &

# 开机自启并后台运行
编辑 /etc/crontab 文件, 并添加下面一行并多按几个回车. (有些系统不留空行会出现意外)
@reboot root nohup /path/to/OneList -bind 0.0.0.0 -port 5288 >/dev/null 2>&1 &
```

## 12、Client/Secret (~ 2299/12/31)
...
service firewalld start       开启防火墙（记得将常用端口都开放）

firewall-cmd --zone=public --add-port=5288/tcp --permanent 

Ubuntu 防火墙放行端口：

sudo ufw enable               开启防火墙（记得将常用端口都开放）
